...
export const useAnalyticsStore = defineStore('analytics', {
  state: () => ({
    machines: {},
    shifts: [], // State to hold shifts
    allShifts: [], // State to hold all available shifts
    timestamps: [], 
    selectedMachine: '',
    selectedDate: '',
    selectedShift: '',
    ...
    oeeSeriesData: [], // State to hold OEE series data
  ...
  actions: {
    async fetchMachines() {
      try {
        ...
        this.machines = response.data.reduce((acc, { machine_name, date, shift }) => {
          if (!acc[machine_name]) {
            acc[machine_name] = { dates: {} };
          }
          if (!acc[machine_name].dates[date]) {
            acc[machine_name].dates[date] = [];
          }
          if (!acc[machine_name].dates[date].includes(shift)) {
            acc[machine_name].dates[date].push(shift);
          }
          return acc;
        }, {});
        this.updateShifts();
    ...
    async fetchAllShifts() {
      try {
        ...
        // Extract the shift information from the response data
        const shifts = response.data.map(item => item.shift);
        
        // Remove duplicates from the shifts array
        this.allShifts = [...new Set(shifts)];
      } catch (error) {
    ...
    async fetchShifts() {
      if (this.selectedMachine && this.selectedDate) {
        this.updateShifts();
      } else {
        await this.fetchAllShifts(); // Fetch all shifts if machine or date is not selected
      }
    },

    updateShifts() {
      const machine = this.machines[this.selectedMachine];
      if (machine && machine.dates[this.selectedDate]) {
        this.shifts = machine.dates[this.selectedDate];
      } else {
        this.shifts = this.allShifts; // Use all available shifts
      }
    },
  
    async fetchMetrics() {
      if (!this.selectedMachine || !this.selectedDate || !this.selectedShift) return;
    
      try {
        const response = await axios.get('http://172.18.100.214:8084/cnc_machine_data/filter/', {
          params: {
            machine_name: this.selectedMachine,
            date: this.selectedDate,
            shift: this.selectedShift,
          },
        });
        const data = response.data;
        console.log('fetchMetrics response:', data); // Log response
        ...
        let count = data.length;
           ...
           // Populate series data arrays
           this.availabilitySeriesData = data.map(row => (row.availability * 100).toFixed(2)); // Convert to percentage
           this.performanceSeriesData = data.map(row => (row.performance * 100).toFixed(2)); // Convert to percentage
           this.qualitySeriesData = data.map(row => (row.quality * 100).toFixed(2)); // Convert to percentage
           this.oeeSeriesData = data.map(row => (row.oee * 100).toFixed(2)); // Convert to percentage
             this.timestamps = data.map(row => row.timestamp); // Add timestamps
       // Log the converted data along with timestamps
       console.log('Timestamps:', this.timestamps);
        ...
        data.forEach(row => {
          ...
          totalOEE += row.oee;
          totalRuntime += row.operating_time;
          totalIdleTime += row.plandown_time;
          // Assuming off time can be derived from some calculation
          totalOffTime += row.offtime || (100 - row.operating_time - row.plandown_time);
          totalBadParts += row.bad_parts;
          totalGoodParts += row.good_units;
          totalPartCount += row.part_count;
        });
    
        // Compute averages
        this.metricsData = {
          availability: (totalAvailability / count) * 100,
          performance: (totalPerformance / count) * 100,
          quality: (totalQuality / count) * 100,
          oee: (totalOEE / count) * 100,
          runtime: totalRuntime / count,
          idletime: totalIdleTime / count,
          offtime: totalOffTime / count,
          badparts: totalBadParts / count,
          goodparts: totalGoodParts / count,
          partcount: totalPartCount / count,
        };
  
    console.log('OEE Series Data:', this.oeeSeriesData);
    
        this.oeeSeriesData = this.oeeSeriesData.map(value => parseFloat(value));
    ...
    async updateMetrics() {
      await this.fetchShifts(); // Ensure shifts are updated
